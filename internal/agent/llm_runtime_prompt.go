...
var builtinNativeToolDescs = map[string]string{
	...
	"ask-user":           "Ask the user a question and wait for their answer. Available in every interactive mode (not just planning): use it when a decision is genuinely the user's to make and proceeding on a guess would waste work. Give concrete options rather than an open question; mark the one you would pick with default_option, which the UI highlights as recommended and pre-selects. Set allow_free_response when a written answer is useful — the user then gets a \"type my own answer\" entry alongside your options and their text comes back verbatim. With no options the question is free-text only, and a client that cannot collect text will return an error instead of an answer.",
...
var builtinNativeToolSchemas = map[string]json.RawMessage{
	...
	"ask-user":           json.RawMessage(`{"type":"object","properties":{"question":{"type":"string","description":"the question to put to the user"},"options":{"type":"array","items":{"type":"string"},"description":"selectable answers; prefer these over an open question"},"context":{"type":"string","description":"one line of background shown under the question"},"default_option":{"type":"string","description":"the option you recommend, matched by label; it is marked recommended and pre-selected"},"allow_free_response":{"type":"boolean","description":"also offer a free-text entry; the typed answer is returned verbatim"}},"required":["question"]}`),
