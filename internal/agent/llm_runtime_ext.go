...
// runDevinSession delegates the task to a fresh Cognition / Devin session
// via provider.Manager.CallDevin and returns the agent's final message body
// (plus an app.devin.ai session link) as the tool result. This is the
// "too-complex-for-me, let Devin handle it" escape hatch: the calling
// Spettro agent stays in charge of conversation history, tool orchestration,
// and final user-facing answers; only the specific subtask gets run by
// Devin, with its own ACU billing and its own tool set.
//
// Arguments:
//
//	{
//	  "task":             "<what Devin should do>",
//	  "constraints"?:     "<optional guardrails (must use X, avoid Y)>",
//	  "expected_output"?: "<what the caller wants back (a PR url, a diff summary, ...)>"
//	}
//
// The Devin call blocks until the session finishes or hits a terminal
// status; ctx cancellation stops the polling loop but the Devin-side
// session keeps running and must be cancelled from app.devin.ai.
func (r *toolRuntime) runDevinSession(ctx context.Context, rawArgs []byte) (string, error) {
	var args struct {
	...
	if err := decodeJSONStrict(rawArgs, &args); err != nil {
		return "", fmt.Errorf("devin-session args: %w", err)
	}
	task := strings.TrimSpace(args.Task)
	if task == "" {
		return "", fmt.Errorf("devin-session: task is required")
	}
	if r.providerMgr == nil {
		return "", fmt.Errorf("devin-session: provider manager not configured")
	}
	// Build a compact, self-contained prompt for Devin. Devin's session
	// starts from scratch; it doesn't see Spettro's conversation history.
	...
	if eo := strings.TrimSpace(args.ExpectedOutput); eo != "" {
		b.WriteString("\n\nExpected output:\n")
		b.WriteString(eo)
	}
	out, err := r.providerMgr.CallDevin(ctx, b.String())
	if err != nil {
		return "", fmt.Errorf("devin-session: %w", err)
	}
	return out, nil
}
